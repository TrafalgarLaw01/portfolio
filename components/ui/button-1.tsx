...
export const ContactCTAButton = ({ text, href }: { text: string, href: string }) => {
    const [isHovered, setIsHovered] = useState(false);
    return (
        <div className="flex justify-center">
            <Link
                href={href}
                className="relative inline-block w-48 h-12 mx-auto group dark:bg-black bg-white dark:border-white/20 border-black/20 border rounded-full">
                ...
                <button
                    className="absolute inset-0 rounded-full bg-transparent cursor-pointer flex items-center justify-center pointer-events-none"
                    aria-label={text}
                    type="button"
                    onMouseEnter={() => setIsHovered(true)}
                    onMouseLeave={() => setIsHovered(false)}>
                    <span className="flex items-center justify-center gap-2 rounded-full group-hover:text-cyan-400 text-white text-lg font-bold tracking-wide whitespace-nowrap transition-colors z-20 relative">
                        <span>{text}</span>
                        <ArrowRight className="group-hover:translate-x-1 transition-transform w-5 h-5 flex-shrink-0" />
                    </span>
                </button>
            </Link>
        </div>
